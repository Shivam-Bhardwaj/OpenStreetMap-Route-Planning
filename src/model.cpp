...
void Model::LoadData(const std::vector <std::byte> &xml) {
  ...
  for (const auto &node: doc.select_nodes("/osm/node")) {
    node_id_to_num[node.node().attribute(
      "id").as_string()] = (int) m_Nodes.size();
  ...
  for (const auto &way: doc.select_nodes("/osm/way")) {
    ...
    for (auto child: node.children()) {
      ...
      if (name == "nd") {
        ...
        if (category == "highway") {
          if (auto road_type = String2RoadType(type); road_type !=
                                                      Road::Invalid) {
        ...
        if (category == "railway") {
          ...
          if (auto landuse_type = String2LanduseType(type); landuse_type !=
                                                            Landuse::Invalid) {
...
Track(std::vector<int> &open_ways, const Model::Way *ways) {
  ...
  auto process = [&](std::vector<int> &ways_nums) {
    ...
    while (!open.empty()) {
      ...
      if (new_nodes.empty())
        break;
      open.erase(
        std::remove_if(open.begin(), open.end(), [](auto v) { return v < 0; }),
        open.end());
